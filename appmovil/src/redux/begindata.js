    ...
    switch (stateType){
        ...
        case 'reset_password':
            if (params.reset){ 
                ...
                try{
                    const response = await axios.post (`${constantes().url_principal[0].url}/${params.path}`, params.data)
        ...
        case 'logout':
            if (params.reset){ 
                return {success: false}
                try{
                    const response = await axios.get (`${constantes().url_principal[0].url}/${params.path}`)
                    return response.data
                }catch (err){
                    return err.message
                }
